...
def build_model(regularizer):
    ...
    model.add(tf.keras.layers.Conv2D(filters = 128,
            kernel_size = (2, 2),  activation = 'relu'))
    # We put our regularizer here, after the output of second max pooling
    if (regularizer != 'none' and regularizer != 'dropout'):
        model.add(tf.keras.layers.MaxPooling2D(pool_size = (2, 2), activity_regularizer = regularizer))
...
def run_scenario(regularizer):
    ...
    if (regularizer == 'dropout'):
        ...
        model = build_model(regularizer = tf.keras.regularizers.l2(l=0.001))
